...
const processOpenapiSync = async (...args: Parameters<typeof OpenapiSync>) => {
	...
	const parseSchemaToPythonType = (
		...
		if (schema) {
			if (schema.$ref) {
				...
				type += `Union[${schema.anyOf
					.map((v) =>
						parseSchemaToPythonType(apiDoc, v, "", isRequired, {
							...options,
							useComponentName: false,
						}),
					)
				...
				type += `List[${parseSchemaToPythonType(apiDoc, schema.items, "", false, { ...options, useComponentName: false })}]`;
			} else if (schema.properties) {
				const objKeys = Object.keys(schema.properties);
				const requiredKeys = schema.required || [];
				let typeCnt = "";
				...
				if (nonNullValues.length > 0)
					type += `Literal[${nonNullValues.join(", ")}]`;
				else type += `Any`;
			} else if (schema.type) {
				const handleType = (_type: typeof schema.type) => {
					let typeCnt = "";
					if (typeof _type === "string") {
						if (["integer", "number"].includes(_type))
							typeCnt += _type === "number" ? `float` : `int`;
						else if (_type === "string") typeCnt += `str`;
						else if (_type === "boolean") typeCnt += `bool`;
						else if (_type === "array") typeCnt += `List[Any]`;
						else if (_type === "object") typeCnt += `Dict[str, Any]`;
						else if (_type === "null") typeCnt += `None`;
						else typeCnt += `Any`;
					} else if (Array.isArray(_type)) {
						const arrType = _type
							.map((v) => handleType(v))
							.filter((v) => v !== "");
						if (arrType.length > 1) typeCnt += `Union[${arrType.join(", ")}]`;
						else typeCnt += arrType[0] || "Any";
					} else typeCnt += "Any";
					return typeCnt;
				};
				type = handleType(schema.type);
		...
		let typeName = "";
		if (_name) {
			let validName = _name.replace(/[^a-zA-Z0-9_]/g, "_");
			typeName = `    ${validName}: `;
		}
	...
	const processSchemas = (
		...
		contentKeys.forEach((contentKey) => {
			...
			const typeCnt = isPython
				? `${parseSchemaToPythonType(spec, schema, "", true, {
						noSharedImport: true,
						useComponentName: false,
					})}`
				: `${parseSchemaToType(spec, schema, "", true, {
						noSharedImport: true,
						useComponentName: false,
					})}`;
	...
	const getBodySchemaType = (requestBody: IOpenApiRequestBodySpec) => {
		...
		if (requestBody.content) {
			...
			if (contentKeys[0] && requestBody.content[contentKeys[0]].schema) {
				typeCnt += isPython
					? `${parseSchemaToPythonType(spec, requestBody.content[contentKeys[0]].schema as IOpenApSchemaSpec, "")}`
					: `${parseSchemaToType(spec, requestBody.content[contentKeys[0]].schema as IOpenApSchemaSpec, "")}`;
			}
		}
		return typeCnt;
	};
...
--cacert ca-certificate.crt`;
										...
										headers[securitySchema?.name || "X-API-KEY"] =
											`{API_KEY_VALUE}`;
									} else {
